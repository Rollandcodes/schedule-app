...
const IS_EMBED_TEST = process.argv.some((a) => a.startsWith("--project=@schedule/embed-core"));
const IS_EMBED_REACT_TEST = process.argv.some((a) => a.startsWith("--project=@schedule/embed-react"));

// Suppress all webServer logs to reduce noise during E2E tests
const webServer: PlaywrightTestConfig["webServer"] = [
  {
    command:
      "yarn workspace @schedule/web copy-app-store-static && NEXT_PUBLIC_IS_E2E=1 NODE_OPTIONS='--dns-result-order=ipv4first' yarn workspace @schedule/web start -p 3000",
    port: 3000,
    timeout: 60_000,
...
if (IS_EMBED_TEST) {
  ...
  webServer.push({
    command: "yarn workspace @schedule/embed-core dev",
    port: 3100,
    timeout: 60_000,
    reuseExistingServer: !process.env.CI,
    stdout: "ignore",
...
if (IS_EMBED_REACT_TEST) {
  ...
  webServer.push({
    command: "yarn workspace @schedule/embed-react dev",
    port: 3101,
    timeout: 60_000,
    reuseExistingServer: !process.env.CI,
    stdout: "ignore",
...
const config: PlaywrightTestConfig = {
  ...
  projects: [
    ...
    {
      name: "@schedule/embed-core--firefox",
    ...
    {
      name: "@schedule/embed-core--webkit",
      testDir: "./packages/embeds/",
      testMatch: /.*\.e2e\.tsx?/,
      expect: {
        timeout: DEFAULT_EXPECT_TIMEOUT,
    ...
    {
      name: "@schedule/embed-core--isMobile",
      testDir: "./packages/embeds/embed-core/",
      testMatch: /.*\.e2e\.tsx?/,
      expect: {
        timeout: DEFAULT_EXPECT_TIMEOUT,
