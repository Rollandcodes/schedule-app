...
import type { BookingType } from "@schedule/features/bookings/lib/handleNewBooking/originalRescheduledBookingUtils";
...
import { getPiiFreeCalendarEvent } from "@schedule/lib/piiFreeData";
...
import type { Prisma, User } from "@schedule/prisma/client";
import type { SchedulingType } from "@schedule/prisma/enums";
import type { EventTypeMetadata } from "@schedule/prisma/zod-utils";
import type { AdditionalInformation, CalendarEvent, Person } from "@schedule/types/Calendar";
...
export class BookingEmailSmsHandler {
  ...
  private async _handleRescheduled(data: RescheduleEmailAndSmsPayload) {
    ...
    const { sendRescheduledEmailsAndSMS } = await import("@schedule/emails/email-manager");
  ...
  private async _handleRoundRobinRescheduled(data: RescheduleEmailAndSmsPayload) {
    ...
    } = await import("@schedule/emails/email-manager");
  ...
  private async _handleConfirmed(data: ConfirmedEmailAndSmsPayload) {
    ...
    const { sendScheduledEmailsAndSMS } = await import("@schedule/emails/email-manager");
  ...
  public async handleAddAttendee(data: AddGuestsEmailAndSmsPayload) {
    ...
    const { sendAddGuestsEmailsAndSMS } = await import("@schedule/emails/email-manager");
