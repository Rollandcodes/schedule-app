...
vi.mock("@schedule/features/auth/lib/verifyCodeUnAuthenticated", () => ({
...
vi.mock("@schedule/features/users/repositories/UserRepository", async (importOriginal) => {
  ...
  describe("User Email Verification Setting", () => {
    ...
    test("should create booking when main booker provides valid verification code", async () => {
      ...
      const { verifyCodeUnAuthenticated } = await import(
        "@schedule/features/auth/lib/verifyCodeUnAuthenticated"
      );
