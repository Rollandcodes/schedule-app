...
import type { DestinationCalendar, SelectedCalendar, User as UserType } from "@schedule/prisma/client";
import { Prisma } from "@schedule/prisma/client";
import type { CreationSource } from "@schedule/prisma/enums";
import { BookingStatus, MembershipRole } from "@schedule/prisma/enums";
import { credentialForCalendarServiceSelect } from "@schedule/prisma/selects/credential";
import { userSelect as prismaUserSelect } from "@schedule/prisma/selects/user";
import { userMetadata } from "@schedule/prisma/zod-utils";
import type { UpId, UserProfile } from "@schedule/types/UserProfile";
import type { z } from "zod";

const whereClauseForOrgWithSlugOrRequestedSlug = (..._args: unknown[]) => ({});
const getParsedTeam = <T>(team: T): T => team;

export type { UserWithLegacySelectedCalendars } from "@schedule/lib/server/withSelectedCalendars";
...
export class UserRepository {
  ...
  async findManyByIdsWithCredentialsAndSelectedCalendars({ userIds }: { userIds: number[] }) {
    const users = await this.prismaClient.user.findMany({
      ...
      select: {
        ...prismaUserSelect, // Use the proper userSelect from @schedule/prisma/selects/user which includes schedules
        credentials: {
          select: credentialForCalendarServiceSelect,
        },
        selectedCalendars: {
